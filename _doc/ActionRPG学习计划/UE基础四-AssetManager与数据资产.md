...
> 本篇偏实操（动手加一个新物品类型）。如果你对几个概念还夹生——**Asset Manager 到底有哪些用途、Primary Data Asset 是什么、它和 Asset Manager 在源码层面什么关系、有 `FPrimaryAssetType` 那有没有 Secondary**——配套答疑见 [阶段四·补充：把 Asset Manager 这几个概念彻底讲通](UE基础四-补充-AssetManager核心概念答疑.md)。建议先读正文跑通实操，再回补充篇把概念钉死。
